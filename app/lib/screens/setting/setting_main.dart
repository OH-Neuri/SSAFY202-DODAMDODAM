                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      Padding(
                        padding:
                            const EdgeInsets.only(left: 18.0, bottom: 14.0),
                      ...
                      for (int i = 0; i < nc.notifyList.length; i++)
                        InkWell(
                          onTap: () {
                            ...
                            if (nc.notifyList[i].type == 1) {
                              try {
                                NoticeController.to
                                    .setNoticeDetail(nc.notifyList[i].typeSeq);
                                nc.readAlarm(nc.notifyList[i].notifySeq,
                                    nc.notifyList[i].receiveUserSeq, i);
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) =>
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                                ...
                                border: Border(
                                    bottom: BorderSide(
                                        color: Color(0x80FFCF2C), width: 0.4))),
                            child: Row(
                              children: [
                                Padding(
                                  padding: const EdgeInsets.only(right: 24.0),
                                  child: Image.asset(
                                    'assets/images/common/벨.png',
                                    height: 30,
                                  ),
                                ),
                                Expanded(
                            ...
                            child: dc.isTeacher
                                ? CircleAvatar(
                                    backgroundImage: AssetImage(
                                        'assets/images/common/flower_icon.png'),
                                  )
                                : CircleAvatar(
                                    backgroundImage: NetworkImage(dc.kidPhoto),
                                  )),
                        Padding(
                          padding:
                              const EdgeInsets.only(top: 10.0, bottom: 4.0),
                          child: Text('${dc.name} 님',
                              style: TextStyle(
                            ...
                            onTap: () {
                              if (!dc.isTeacher) {
                                CustomSnackBar.errorSnackbar(
                                    context, '방해금지모드는 선생님만 이용할 수 있습니다.');
                                return;
                              }
                              Navigator.of(context).push(MaterialPageRoute(
                                  builder: (context) => SleepModePage()));
                            },
                            child: Container(
                              decoration: BoxDecoration(
                                color: Colors.white,
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        Text(
          text,
          style: TextStyle(fontSize: contentTextSize, color: Colors.grey[800]),
        )
      ],
    );
  }
