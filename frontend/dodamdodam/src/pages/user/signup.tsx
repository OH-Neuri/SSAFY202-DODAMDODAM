...
import { checkAuthSMS, sendAuthSMS, userSignup } from '@/api/user/default';
...
export default function Signup() {
    ...
    const [idRule, setIdRule] = useState<string>('영문 또는 영문/숫자 조합으로 6~12자')
        ...
        <div className='flex justify-center items-center h-screen bg-b_yellow'>
            ...
            <div className='flex w-[1200px] h-[680px] bg-white rounded-xl'>
                ...
                <div className='flex flex-col items-center pt-8 px-16 w-1/2 h-full bg-m_yellow rounded-r-xl'>
                    ...
                    <div className=''>{}</div>
                    <div onClick={()=>{signup()}} className='flex justify-center items-center w-full h-[70px] bg-white/90 hover:bg-white cursor-pointer font-preM text-[20px] rounded-md'>생성하기</div>
                    </>
                    :
                    <>
                    ...
                    <div className='w-full mt-16'>
                        ...
                        <div className='flex items-center mt-2 mb-8 '>
                            ...
                            <div onClick={()=>{sendAuthPhone()}} className='flex justify-center items-center cursor-pointer bg-yellow-500 w-[30%] h-[50px] rounded ml-3 font-preR hover:bg-yellow-600/60'>인증번호 받기</div>
                        </div>
                        {isSend ?
                        <div className='h-[180px]'>
                            <div className='font-preR text-grey-800'>인증번호</div>
                            <div className='flex items-center rounded w-full mt-2 mb-4'>
                                <input onChange={(e)=>{setCode(e.target.value)}} className='outline-none bg-white/70 rounded w-full mt-2 mb-12 h-[50px] px-4' type='text'></input>
                            </div>
                        </div> :
                    ...
                    <div className=''>{}</div>
                    <div onClick={()=>authPhoneNumber()} className='flex justify-center items-center w-full h-[70px] bg-white/90 hover:bg-white cursor-pointer font-preM text-[20px] rounded-md'>다음</div>
                    <div className='mt-12'>
