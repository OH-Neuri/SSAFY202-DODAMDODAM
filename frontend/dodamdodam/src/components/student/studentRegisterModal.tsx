...
export default function StudentRegisterModal(props: {
  ...
  async function getRegisterKidInfo() {
    ...
    try {
      ...
      const response = await axios.post('https://dodamdodam.site/api/dodam/kindergarten/kid', formData, config);
    } catch (error) {
      console.log(error);
                    ...
                    >
                      {
                        classList.map((v, i) => {
                          return (
                            <MenuItem key={i} value={v.classSeq}>{v.className}</MenuItem>
                          )
                        })
                      }
                    </Select>
